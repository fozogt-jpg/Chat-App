...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        try {
            ...
            return; // ignore malformed frames instead of crashing the connection
        }
                ...
                [serverId, username, text, avatar], (err) => {
                    if (!err) {
                        wss.clients.forEach(client => {
                            // FIX: `WebSocket` was never imported (only `Server` was destructured
                            // from 'ws'), so `WebSocket.OPEN` used to throw a ReferenceError here
                            // and silently kill the broadcast. Each client socket exposes its own
                            // `.OPEN` constant, so we use that instead.
                            if (client.readyState === client.OPEN && client.serverId === serverId) {
